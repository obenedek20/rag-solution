...
# print("Creating semantic splitter node parser...")
# splitter = SemanticSplitterNodeParser(
#     embed_model=embed_model,
#     breakpoint_percentile_threshold=95, # how sensitive alg is to content changes - sets the percentile of cosine dissimilarity that must be exceeded between adjacent sentence groups to trigger a new node
#     # 90 - larger chunks/fewer splits, 95 - balanced (standard), 97-99 - very small, highly granular chunks
#     include_metadata=True # include metadata in nodes
# ) # can also tune: buffer_size (# sentences to group together when evaluating semantics), include_prev_next_rel (include previous and next relationships)) 
...
4. Breaks when similarity drops below threshold
"""
...
# print("Encoding texts into embeddings for global embeddings...")
# embeddings = model.encode(
#     texts,
#     convert_to_numpy=True,
#     show_progress_bar=True,
#     normalize_embeddings=True  # important for cosine similarity, improves retrieval stability
# )

# Builds per-company FAISS index for similarity search

# Builds FAISS index for similarity search
# dim = embeddings.shape[1] # n-dimensional vectors of embedding model

# index = faiss.IndexFlatIP(dim)  # creates empty vector db, cosine similarity via inner product
# index.add(embeddings.astype(np.float32)) # embeddings added to vector db
# faiss.write_index(index, "index.faiss") # binary file created - can now be referenced
# print(f"Saved FAISS index with {index.ntotal} vectors to index.faiss")
